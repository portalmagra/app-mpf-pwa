...
export default function EbooksDietasPage() {
  ...
  useEffect(() => {
    const loadEbooks = async () => {
      try {
        console.log('🔄 Carregando eBooks de Dietas...')
        const dietasEbooks = await ebookService.getEbooksByCategory('dietas')
        
        console.log('📦 eBooks de Dietas carregados:', dietasEbooks.length)
        setEbooks(dietasEbooks)
        setLoading(false)
      } catch (error) {
        console.error('❌ Erro ao carregar eBooks de Dietas:', error)
        setLoading(false)
      }
    }
  ...
  const getDietEmoji = (title: string) => {
    const titleLower = title.toLowerCase()
    if (titleLower.includes('mediterrânea')) return '🌊'
    if (titleLower.includes('low carb')) return '🥑'
    if (titleLower.includes('keto')) return '🥓'
    if (titleLower.includes('detox')) return '🥤'
    if (titleLower.includes('vegetariana')) return '🥬'
    if (titleLower.includes('vegana')) return '🌱'
    if (titleLower.includes('intermitente')) return '⏰'
    if (titleLower.includes('dash')) return '❤️'
    return '📚'
  }
    ...
    return (
      <div className="min-h-screen bg-brand-cream flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-brand-text">Carregando eBooks de Dietas...</p>
        </div>
      </div>
    )
  }
      ...
      <main className="max-w-sm mx-auto px-4 py-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-brand-textSoft mt-2">
            Guias completos de diferentes tipos de dietas
          </p>
        </div>
        ...
        <div className="mb-6">
          <input
            type="text"
            placeholder="Buscar eBooks de dietas..."
            value={searchTerm}
                  ...
                  <div className="flex-shrink-0">
                    {ebook.cover_image_url ? (
                      ...
                      <div className="w-20 h-28 bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg flex items-center justify-center">
                        <span className="text-3xl">{getDietEmoji(ebook.title)}</span>
                      </div>
                    )}
                  </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-brand-textSoft text-sm">
              {searchTerm || activeFilter !== 'todos' 
                ...
                : 'Em breve teremos novos eBooks de dietas disponíveis'
              }
            </p>
          </div>
        )}
